...
const CompareSection: React.FC = () => {
  // Use two popular gadgets for the showcase
  const gadgetA = gadgets.find(p => p.id === '5') || gadgets[0]; // iPhone 15 Pro Max
  const gadgetB = gadgets.find(p => p.id === '1') || gadgets[1]; // Samsung Galaxy A54 5G (Good for contrast)

  const comparisonSpecs = [
    { label: 'Battery Performance', icon: Battery, valueA: 'Up to 29h Video', valueB: 'Up to 21h Video', winner: 'A' },
    { label: 'Camera Quality', icon: Camera, valueA: 'Pro 48MP System', valueB: '50MP Triple Cam', winner: 'A' },
    { label: 'Processing Power', icon: Cpu, valueA: 'A17 Pro (3nm)', valueB: 'Exynos 1380 (5nm)', winner: 'A' },
    { label: 'Display Refresh', icon: Zap, valueA: '120Hz ProMotion', valueB: '120Hz Super AMOLED', winner: 'Both' },
  ];
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 md:mb-24">
           <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-zinc-100 text-zinc-900 rounded-full text-[11px] font-black uppercase tracking-[0.2em] mb-6 shadow-sm">
              <GitCompare size={14} className="text-primary" /> Powerful Data Comparison
           </div>
           <h2 className="text-4xl md:text-6xl font-black text-dark tracking-tight leading-tight mb-6">
             Compare Gadgets <br /> <span className="text-primary">Side by Side.</span>
           </h2>
           <p className="text-muted font-medium text-lg leading-relaxed">
             Stop guessing and start knowing. Our comparison engine helps you visualize every spec difference with absolute clarity.
           </p>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-[3rem] border border-zinc-200 shadow-2xl shadow-primary/5 overflow-hidden">
             ...
             <div className="flex flex-col md:flex-row">
                ...
                <div className="flex-1 p-8 md:p-12 flex flex-col items-center bg-zinc-50/30">
                   ...
                   <div className="flex items-center gap-1.5 text-xs font-black text-emerald-600 uppercase tracking-widest mb-6">
                      <ShieldCheck size={14} /> Market Leader
                   </div>
                </div>
                ...
                {comparisonSpecs.map((spec, i) => (
                  <div key={i} className="flex items-stretch border-b border-zinc-100 last:border-0 hover:bg-zinc-50/50 transition-colors">
                     ...
                     <div className="flex-1 p-6 md:p-8 text-center flex flex-col justify-center">
                        ...
                        {spec.winner === 'B' && <span className="text-[8px] font-black text-emerald-500 uppercase mt-1 tracking-widest">WINS</span>}
                     </div>
                  </div>
                ))}
             </div>
             ...
             <div className="p-8 md:p-12 bg-zinc-900 flex flex-col md:flex-row items-center justify-between gap-8">
                ...
                >
                  Start New Comparison <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                </Link>
             </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-20 md:mt-24">
           {[
             { label: 'Real-time Specs', desc: 'Pulled from verified sources' },
             { label: 'Direct Prices', desc: 'Aggregated across top stores' },
             { label: 'Community Input', desc: 'Integrated user sentiments' },
             { label: 'Expert Insight', desc: 'Professional review scores' }
           ].map((feat, i) => (
