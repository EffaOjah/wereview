                    ...
                    <div id={`review-${review.id}`} key={review.id} className="flex flex-col md:flex-row gap-6 border-b border-zinc-100 pb-10 last:border-0 last:pb-0 px-4 -mx-4 pt-4 -mt-4">
                      ...
                      <div className="md:w-1/4 flex flex-col gap-3">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-full overflow-hidden border border-zinc-100 flex items-center justify-center bg-zinc-50">
                            <img 
                              src={review.user?.avatar ? getImageUrl(review.user.avatar) : `https://ui-avatars.com/api/?name=${encodeURIComponent(review.user?.name || review.author || 'U')}&background=0066ff&color=fff&bold=true`} 
                              alt={review.user?.name || review.author || 'User'} 
                              className="w-full h-full object-cover"
                            />
                          </div>
                          <div className="flex flex-col">
                            <Link to={`/user/${review.userId || review.authorId}`} className="font-black text-primary leading-tight underline transition-all">
